...
class Mesh {
   public:
    struct Vertex {
        Vertex(vec3 p, vec2 tc, vec3 n, vec3 t) : pos(p), uv(tc), norm(n), tangent(t) {}
        vec3 pos;
        vec2 uv;
        vec3 norm;
        vec3 tangent;
    ...
    // Information about a mesh object
    struct MeshObject {
        MeshObject() {
            n_Indices = 0;
            baseVertex = 0;
    ...
    struct Material {
        ...
        Texture* normTex = NULL;  // normal map
    };
    ...
    struct Animation {
        ...
        vec3 interpolateScale(float atime) {
            ...
            float deltaTime = scalingKeys[scalingIndex + 1].w - scalingKeys[scalingIndex].w;  // delta between two adjacent scaling keyframes
    ...
    std::vector<vec3> tangents;         // vertex tangents
    std::vector<vec3> bitangents;       // vertex bitangents
