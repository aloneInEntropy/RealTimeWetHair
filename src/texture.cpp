...
bool Texture::loadCubemap(std::vector<std::string> faces) {
    ...
    int tw, th, tn;
    unsigned char* data = stbi_load(faces[0].c_str(), &tw, &th, &tn, 0);
    glTextureStorage2D(texture, 1, GL_RGBA8, tw, th);
    ...
    for (int i = 0; i < faces.size(); i++) {
        ...
        if (data) {
            ...
            switch (nrChannels_) {
                case 1:
                    glTextureSubImage3D(texture, 0, 0, 0, i, width_, height_, 1, GL_RED, GL_UNSIGNED_BYTE, data);
                    break;
                case 3:
                    glTextureSubImage3D(texture, 0, 0, 0, i, width_, height_, 1, GL_RGB, GL_UNSIGNED_BYTE, data);
                    break;
                case 4:
                    glTextureSubImage3D(texture, 0, 0, 0, i, width_, height_, 1, GL_RGBA, GL_UNSIGNED_BYTE, data);
                    break;
                default:
                    printf("unsupported image bits per pixel");
                    break;
    ...
    glTextureParameteri(texture, GL_TEXTURE_MIN_FILTER, GL_NEAREST);    // minification
    glTextureParameteri(texture, GL_TEXTURE_MAG_FILTER, GL_NEAREST);    // magnification
    glTextureParameteri(texture, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);  // u coordinate (x)
    glTextureParameteri(texture, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);  // v coordinate (y)
    glTextureParameteri(texture, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);  // w coordinate (z)
    return glGetError() == GL_NO_ERROR;
}
